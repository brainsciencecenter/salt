...
gcloud compute instances describe holder-cluster-controller --zone="$zone" | sed -n -e '/disks/,/id:/ p' | grep deviceName: | cut -f 2 -d : | sed "s/$/ $zone/" | while read DISK_NAME ZONE; do
