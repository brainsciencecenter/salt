...
Projects     = sorted(set(
                 pyjq.all('.[].Partitions[].InstanceParameters | .Project | select(. != null)', ClusterConfig)
               + pyjq.all('.[].Controller.Project', ClusterConfig)))
...
def start_instances(compute, node_list):
    ...
    for node in node_list:
        ...
        PartitionsConfig = pyjq.all('.[].Partitions', ClusterConfig)[0]
        Partition = getNodePartition(node, PartitionsConfig)
    ...
    try:
        ...
        while True:
            for project in Projects:
                ...
                zonelist = pyjq.all('..| objects | select(.Project == "{}") | select(.Zone != null) | .Zone'.format(project), ClusterConfig)
                if (debug):
                    print("Project = {}, Zone = {}".format(project,zonelist))
