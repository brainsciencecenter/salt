{% set AppsDir = '/share/apps' %}
{% set SlurmDir = AppsDir + '/slurm' %}
{% set DownloadDir = '/share/apps/slurm/src' %}
...
{% set SlurmctldLogFile = pillar.get('slurm:SlurmctldLogFile', '/share/apps/slurm/log/slurm.log') %}
{% set SlurmDebug = pillar.get('slurm:SlurmDebug', 'debug') %}
{% set SlurmdLogFile = pillar.get('slurm:LogFile', '/share/apps/slurm/log/slurm.log') %}
...
{% set DbdLogFile = pillar.get('slurm:DbdLogFile', '/share/apps/slurm/log/slurmdbd.log') %}
{% set DbdPidFile = pillar.get('slurm:DbdPidFile', '/var/run/slurm/slurmdbd.pid') %}
{% set BuildDir = SrcDir + '/build' %}
{% set SlurmRootDir = '/share/apps/slurm/slurm-19.05.5' %}
{% set SlurmCurrent = '/share/apps/slurm/current' %}
...
{{ SlurmScriptDir }}/cluster.yaml:
  file.managed:
    - source: salt://files{{ SlurmScriptDir }}/cluster.yaml.{{ grains['id'] }}
    - user: root
    - group: root
    - mode: 644
    - makedirs: True
